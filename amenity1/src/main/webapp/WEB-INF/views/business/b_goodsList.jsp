...
<script>
    // JavaScript로 select 변경 이벤트를 감지하고 처리하는 부분
    function handleCompanySelect() {
        var selectedCompany = document.querySelector('select[name="company"]').value;
        var goodsTable = document.getElementById('goodsTable');
        
        if (selectedCompany === '') {
        	goodsTable.style.display = 'none'; // 상품 테이블 숨김
        } else {
        	goodsTable.style.display = 'table'; // 상품 테이블 표시
        }
    }
